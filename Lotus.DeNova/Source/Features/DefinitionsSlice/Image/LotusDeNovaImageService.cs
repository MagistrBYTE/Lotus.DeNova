    ...
    namespace DeNova
	{
		...
		*@{*/
		...
		public class ImageService : ILotusImageService
        {
			...
			//---------------------------------------------------------------------------------------------------------
			public async Task<Response> AddAsync(ImageDto imageUpdate, CancellationToken token)
			{
				Image? entity = await _context.Images.FirstOrDefaultAsync(x => x.Id == imageUpdate.Id, token);
				if (entity == null)
				{
					entity = imageUpdate.Adapt<Image>();
					_context.Images.Add(entity);
					await _context.SaveChangesAsync(token);
				}
            ...
            {
				var entity = imageUpdate.Adapt<Image>();
