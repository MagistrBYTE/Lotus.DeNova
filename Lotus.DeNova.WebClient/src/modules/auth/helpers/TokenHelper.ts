...
export const TokenHelper = {
  ...
  checkUserUserPermission(permission: string): boolean 
  {
    const accesUserPermissions = storage.getItem(TokenItemEnum.UserPermissions);
    if(accesUserPermissions)
    {
      return accesUserPermissions.includes(permission);
    }
    return false;
  },
  ...
  checkUserPermissions(permissions: string[]|undefined): boolean 
  {
    ...
    if(accesUserPermissions)
    {
      for (let index = 0; index < permissions.length; index++) 
      {
        ...
        const exist = accesUserPermissions.includes(permission)
        if(exist)
        {
          return true;
        }
